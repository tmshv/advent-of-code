...
fn main() {
    ...
    const HEAD: usize = 0;
    const TAIL: usize = 9;
    let mut knots: Vec<Vector> = vec![];
    for _ in 0..10 {
        knots.push(Vector { x: 0, y: 0 });
    }

    let mut log: Vec<Vector> = vec![];
    log.push(knots[TAIL].clone());
    ...
    println!("Head end: {:?}", knots[HEAD]);
    println!("Tail end: {:?}", knots[TAIL]);
