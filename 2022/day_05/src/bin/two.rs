...
impl<T> Stack<T> {
    ...
    fn push_many(&mut self, items: Vec<T>) {
        for x in items {
            self.items.push(x);
        }
    }

    fn pop_many(&mut self, n: u32) -> Vec<T> {
        let mut v: Vec<T> = Vec::new();
        for _ in 0..n {
            let x = self.items.pop().unwrap();
            v.push(x);
        }
        v.reverse();
        v
...
fn main() {
    ...
    let out: Vec<_> = tops
        .into_iter()
        .map(|key| head.get(key).unwrap().top().unwrap().clone())
        .collect();
    println!("{}", out.join(""));
}
